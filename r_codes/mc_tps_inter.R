...
margin_of_error <- qnorm((1 + confidence_level) / 2) * (sd_output / sqrt(n_monte))
...
num_boots <- 100
boots_means <- numeric(num_boots)

for (i in 1:num_boots) {
  bootstrap_sample <- sample(mc_solh_median, replace = TRUE)
  boots_means[i] <- mean(bootstrap_sample)
}
...
# Print results
cat("Mean Estimate from Monte Carlo:", mean(mc_solh_median), "\n")
cat("Standard Error from Bootstrapping:", se_boots_means, "\n")
